...
optional arguments:
  ...
  -p photogrammetry/panoramaHdr/panoramaFisheyeHdr/MG_FILE, --pipeline photogrammetry/panoramaHdr/panoramaFisheyeHdr/MG_FILE
                        "photogrammetry" pipeline, "panotamaHdr" pipeline,
                        "panotamaFisheyeHdr" pipeline or a Meshroom file
                        containing a custom pipeline to run on input images.
                        Requirements: the graph must contain one CameraInit
                        node, and one Publish node if --output is set.
...
Example:
./meshroom_batch -i /fsx/users/willfeng/3d_recon/images/key -p photogrammetry -o /fsx/users/willfeng/3d_recon/output/key --save /fsx/users/willfeng/3d_recon/save/key/key_proj.mg