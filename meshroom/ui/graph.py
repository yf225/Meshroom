...
class UIGraph(QObject):
    ...
    def setGraph(self, g):
        ...
        if self._graph:
            self.stopExecution()
            # Clear all the submitted nodes at once before the graph gets changed, as it won't receive further updates
            if self._computingLocally:
                self._graph.clearSubmittedNodes()
            self.clear()
    ...
    def updateGraphComputingStatus(self):
        ...
        computingLocally = any([ch.status.execMode == ExecMode.LOCAL and
                                ch.status.status in (Status.RUNNING, Status.SUBMITTED) and
                                ch.status.sessionUid == sessionUid for ch in self._sortedDFSChunks])
